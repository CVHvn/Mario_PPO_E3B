...
def get_args():
    ...
    parser.add_argument('--save_figure_step', type=int, default=512, help='Number of steps between testing model')
...
def test(config):
    ...
    embedding_model = Embedding_Model(config.embedding_dim, config.use_layer_init)
    inverse_dynamics_model = InverseDynamicsModel(config.embedding_dim, config.action_dim, config.use_layer_init)
    agent = Agent(envs = None, world = config.world, stage = config.stage, action_type = config.action_type, num_envs = config.num_envs,
              additional_bonus_state_8_4_option = config.additional_bonus_state_8_4_option, state_dim = config.state_dim, 
              action_dim = config.action_dim, save_dir = config.save_dir, save_model_step = config.save_model_step, 
              save_figure_step = config.save_figure_step, learn_step = config.learn_step, 
              total_step_or_episode = config.total_step_or_episode, total_step = config.total_step, total_episode = config.total_episode,
              model = model, embedding_model = embedding_model, inverse_dynamics_model = inverse_dynamics_model,
              gamma = config.gamma, gamma_int = config.gamma_int, learning_rate = config.learning_rate, 
              entropy_coef = config.entropy_coef, V_coef = config.V_coef, max_grad_norm = config.max_grad_norm, 
              clip_param = config.clip_param, batch_size = config.batch_size, num_epoch = config.num_epoch, 
              is_normalize_advantage = config.is_normalize_advantage, V_loss_type = config.V_loss_type, target_kl = config.target_kl, 
              gae_lambda = config.gae_lambda, ext_adv_coef = config.ext_adv_coef, int_adv_coef = config.int_adv_coef, 
              update_proportion = config.update_proportion, norm_int_reward = config.norm_int_reward, lambda_C = config.lambda_C, 
              embedding_dim = config.embedding_dim, device = "cuda:0" if torch.cuda.is_available() else "cpu")
