...
class Agent():
    def __init__(self, envs, world, stage, action_type, num_envs, state_dim, action_dim, save_dir, save_model_step,
                 ...
                 embedding_model, inverse_dynamics_model, gamma, gamma_int, learning_rate, entropy_coef, V_coef, max_grad_norm,
                 clip_param, batch_size, num_epoch, is_normalize_advantage, V_loss_type, target_kl, gae_lambda, int_adv_coef,
                 ext_adv_coef, additional_bonus_state_8_4_option, update_proportion, norm_int_reward, lambda_C, embedding_dim, device):
        self.world = world
        self.stage = stage
        ...
        self.inverse_dynamics_model = inverse_dynamics_model.to(self.device)
        ...
        self.embedding_optimizer = torch.optim.Adam(self.embedding_model.parameters(), lr=self.learning_rate)
        self.inverse_dynamics_optimizer = torch.optim.Adam(self.inverse_dynamics_model.parameters(), lr=self.learning_rate)
        ...
        self.norm_int_reward = norm_int_reward
        ...
        self.embedding_loss = np.zeros((1000,)).reshape(-1)
        self.embedding_loss_idx = 0
        self.embedding_loss_len = 0
...
loss_e: {(self.E_loss.sum()/self.len_loss):.4f} loss_embedding: {(self.embedding_loss.sum()/self.embedding_loss_len):.4f} \
...
episode_time: {datetime.now() - episode_time}\n')
    ...
    def update_embedding_log(self, loss_embedding):
        self.embedding_loss[self.embedding_loss_idx] = loss_embedding
        self.embedding_loss_idx = (self.embedding_loss_idx + 1)%1000
        self.embedding_loss_len = min(self.embedding_loss_len+1, 1000)
    ...
    def compute_reward_int(self, next_state, inverse_C):
        ...
        with torch.no_grad():
            embedding = self.embedding_model(next_state)
            embedding = embedding.reshape(embedding.shape[0], embedding.shape[1], 1)

        b = embedding.transpose(1, 2) @ inverse_C @ embedding
        u = inverse_C @ embedding
        inverse_C = inverse_C - 1/(1+b+1e-9) * u @ u.transpose(1, 2)

        return b.reshape(-1).cpu().numpy(), inverse_C
    ...
    def learn(self):
        ...
        if self.norm_int_reward == 'min_max':
            rewards_int_norm = np.array(rewards_int)
        ...
        next_states = next_states[:, 3, :, :].reshape(-1, 1, next_states.shape[2], next_states.shape[3])
        next_states_norm = ((next_states - self.obs_rms.mean) / torch.sqrt(self.obs_rms.var)).clip(-5, 5)
        next_states_norm = next_states_norm.to(self.device)

        states = torch.tensor(np.array(states), device = self.device)
        states = states.reshape((-1,  states.shape[2], states.shape[3], states.shape[4]))
        states_norm = states[:, 3, :, :].reshape(-1, 1, states.shape[2], states.shape[3]).to(self.device)
        states_norm = ((states_norm - self.obs_rms.mean.to(self.device)) / torch.sqrt(self.obs_rms.var.to(self.device))).clip(-5, 5)
        ...
        for epoch in range(self.num_epoch):
            ...
            num_sample_embeddings = int(start_id * self.update_proportion) + int(start_id * self.update_proportion > int(start_id * self.update_proportion))
            shuffle_ids = shuffle_ids[:num_sample_embeddings]
            for i in range(num_sample_embeddings//self.batch_size + int(num_sample_embeddings % self.batch_size != 0)):
                start_id = i * self.batch_size
                end_id = min(len(shuffle_ids), (i+1) * self.batch_size)
                batch_ids = shuffle_ids[start_id:end_id]

                batch_state = states_norm[batch_ids]

                batch_action_index = action_index[batch_ids]

                batch_next_states = next_states_norm[batch_ids]

                emb1 = self.embedding_model(batch_state)
                emb2 = self.embedding_model(batch_next_states)
                predict_action_logits = self.inverse_dynamics_model(emb1, emb2)
                labels = F.one_hot(batch_action_index, self.action_dim).float().to(self.device)

                loss_embedding = F.cross_entropy(predict_action_logits, labels)

                loss_embedding.backward()
                torch.nn.utils.clip_grad_norm_(self.embedding_model.parameters(), self.max_grad_norm)
                ...
                self.update_embedding_log(loss_embedding.item())
    ...
    def train(self):
        ...
        inverse_C = (1./ self.lambda_C) * torch.eye(self.embedding_dim).unsqueeze(0).expand(self.num_envs, -1, -1).to(self.device)
        ...
        while True:
            ...
            rewards_int, inverse_C = self.compute_reward_int(next_states, inverse_C)
...
loss_v_int: {(self.V_int_loss.sum()/self.len_loss):.4f} loss_e: {(self.E_loss.sum()/self.len_loss):.4f} \
loss: {(self.total_loss.sum()/self.len_loss):.4f} approx_kl_div: {(self.approx_kl_divs.sum()/self.len_loss):.4f} \
loss_embedding: {(self.embedding_loss.sum()/self.embedding_loss_len):.4f} \
reward_int: {(self.rewards_int.sum()/self.rewards_int_len):.4f} reward_int_norm: {(self.rewards_int_norm.sum()/self.rewards_int_len):.4f} \
episode_time: {datetime.now() - episode_time[i]} total_time: {datetime.now() - total_time}\n')
                    ...
                    inverse_C[i] = (1./ self.lambda_C) * torch.eye(self.embedding_dim).to(self.device)
